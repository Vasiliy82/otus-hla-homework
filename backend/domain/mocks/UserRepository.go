...
// GetFriendsIds provides a mock function with given fields: id
func (_m *UserRepository) GetFriendsIds(id domain.UserKey) ([]domain.UserKey, error) {
	ret := _m.Called(id)

	if len(ret) == 0 {
		panic("no return value specified for GetFriendsIds")
	}
	...
	if rf, ok := ret.Get(0).(func(domain.UserKey) []domain.UserKey); ok {
		r0 = rf(id)
	} else {
		if ret.Get(0) != nil {
